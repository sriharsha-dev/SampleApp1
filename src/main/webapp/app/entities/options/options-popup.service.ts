            ...
            if (id) {
                this.optionsService.find(id)
                    .subscribe((optionsResponse: HttpResponse<Options>) => {
                        const options: Options = optionsResponse.body;
                        this.ngbModalRef = this.optionsModalRef(component, options);
                        resolve(this.ngbModalRef);
                    });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.optionsModalRef(component, new Options());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
